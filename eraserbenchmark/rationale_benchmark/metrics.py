...
def score_classifications(instances: List[dict], annotations: List[Annotation], docs: Dict[str, List[str]], aopc_thresholds: List[float], num_classes) -> Dict[str, float]:
...
def main():
    ...
    parser.add_argument('--num_classes', dest='num_classes', required=True)
    ...
    if has_final_predictions:
        flattened_documents = load_flattened_documents(args.data_dir, docids)
        class_results = score_classifications(results, annotations, flattened_documents, args.aopc_thresholds, num_classes)
